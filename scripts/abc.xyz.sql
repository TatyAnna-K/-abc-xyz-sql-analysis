...
GROUP BY o.product_id, p.product_name
 ...
 -- Счтитаем кумулятивные суммы для количества проданных товаров, выручки и прибыли:
cumsumma AS (
SELECT product_id,
       product_name,
       quantity,
...
FROM product_summa
 ...
 -- Извлекаем единственное значение коэффициента вариации для каждого товара (в конкретном случае - это технический прием, а не реальный поиск минимального значения):
covars_2 AS (
SELECT product_id,
       product_name,
       MIN(covar) AS cov
...
FROM result_abc ra
JOIN result_xyz rx
ON ra.product_id = rx.product_id
